...
(async function(){
  ...
  const els={from:TC.q('#from'),to:TC.q('#to'),date:TC.q('#date'),time:TC.q('#time'),pax:TC.q('#pax'),estimateBtn:TC.q('#estimate'),calendlyBtn:TC.q('#calendly'),pay20:TC.q('#pay20'),pay100:TC.q('#pay100'),distance:TC.q('#distance'),duration:TC.q('#duration'),total:TC.q('#total'),map:TC.q('#map')};
  els.time.step=300; const now=new Date(); now.setMinutes(now.getMinutes()+120); now.setSeconds(0,0); const step=5; now.setMinutes(Math.ceil(now.getMinutes()/step)*step); els.date.value=now.toISOString().slice(0,10); els.time.value=now.toTimeString().slice(0,5);
  // Build time dropdown (5-min steps)
  ...
  const ds=new google.maps.DirectionsService(); const dr=new google.maps.DirectionsRenderer({map});
  function isNightWeekend(dt){const h=dt.getHours();const isN=(h>=23||h<7);const d=dt.getDay();const isWE=(d===0||d===6);return isN||isWE;}
  function price(dist, dur, when){const min=10, perKm=1.6, perMin=0.55, surge=isNightWeekend(when)?1.2:1; let amt=(perKm*(dist/1000))+(perMin*(dur/60)); if(amt<min) amt=min; return Math.round(amt*surge*100)/100;}
  ...
  // MAD branch: no destination, price by duration
  if(modeMad){
    ...
    // Enable actions
    els.pay20.disabled = false; els.pay100.disabled = false; els.calendlyBtn.disabled = false;
    // Store metadata
  ...
  // Classic/airport branch: need destination
  if(!to){ alert("Renseignez l’adresse d’arrivée."); return; }
  const route = await ds.route({ origin: from, destination: to, travelMode: google.maps.TravelMode.DRIVING, drivingOptions: { departureTime: dt, trafficModel: 'bestguess' } });
  dr.setDirections(route);
  const dm = new google.maps.DistanceMatrixService();
  const r = await dm.getDistanceMatrix({ origins:[from], destinations:[to], travelMode: google.maps.TravelMode.DRIVING, drivingOptions:{ departureTime: dt, trafficModel:'bestguess' } });
  const cell = r.rows[0].elements[0];
  const dist = cell.distance.value;
  const dur = (cell.duration_in_traffic || cell.duration).value;
  // Pricing for classic/airport
  ...
  const cfg = window.cfg || {}; const pk = (cfg.STRIPE_PUBLISHABLE_KEY || (window._TC_CFG && _TC_CFG.STRIPE_PUBLISHABLE_KEY) || (window.TC && TC._config && TC._config.STRIPE_PUBLISHABLE_KEY) || null);
  if(!pk){ alert("Stripe non configuré (clé publishable manquante)."); return; }
  const stripe = window._stripe || (window._stripe = Stripe(pk));
  const elements = window._stripeElements || (window._stripeElements = stripe.elements());
  const card = window._stripeCard || (window._stripeCard = elements.create('card', { hidePostalCode: true }));
  try { card.mount('#card-element'); } catch(e){ /* already mounted */ }
  const amountEur = full ? (window._TC_LAST.price_eur) : Math.max(1, Math.round(window._TC_LAST.price_eur * 0.20 * 100)/100);
  const meta = Object.assign({}, window._TC_LAST||{});
  const status = document.getElementById('card-status');
  status.textContent = "Création du paiement…";
  fetch('/.netlify/functions/create-payment-intent', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount_eur: amountEur, metadata: meta }) })
    .then(async (resp)=>{ if(!resp.ok) throw new Error(await resp.text()); return resp.json(); })
    .then(async ({ clientSecret })=>{
      const result = await stripe.confirmCardPayment(clientSecret, { payment_method: { card, billing_details: { name: 'Client Transport Confort' } } });
      if(result.error){ throw result.error; }
      if(result.paymentIntent && result.paymentIntent.status === 'succeeded'){
        status.style.color = '#2ecc71';
        status.textContent = "✅ Paiement confirmé. Vous pouvez finaliser la réservation dans Calendly.";
      } else {
        status.textContent = "Paiement en attente de validation (3‑D Secure ?)…";
      }
    })
    .catch((e)=>{ console.error(e); status.style.color = '#ff5252'; status.textContent = "❌ Paiement refusé ou interrompu."; });
}catch(e){alert("Paiement indisponible pour le moment.");console.error(e);}
  }
  function calendly(){
    const calMAD=(cfg.CALENDLY_MAD||'').trim(); const calVTC=(cfg.CALENDLY_VTC||window.CALENDLY_URL||'').trim(); const cal = (document.getElementById('mode-mad')?.checked ? calMAD : calVTC); if(!cal){alert("Lien Calendly non configuré.");return;}
    if(!window._TC_LAST){alert("Faites une estimation d'abord.");return;}
    const url=new URL(cal); url.searchParams.set("name","Client Transport Confort"); url.searchParams.set("start_time",window._TC_LAST.whenISO); url.searchParams.set("utm_content",JSON.stringify(window._TC_LAST)); window.open(url.toString(),"_blank");
  }
  els.estimateBtn.addEventListener('click',estimate); els.pay20.addEventListener('click',()=>pay(false)); els.pay100.addEventListener('click',()=>pay(true)); els.calendlyBtn.addEventListener('click',calendly);
})();
...
async function computeAndDisplay(result){
  ...
  // discount 15%
  const discounted = Math.round(pricing.total * 100) / 100;
  ...
  if(panelTotal){
    panelTotal.innerHTML = `<div style="display:flex;flex-direction:column;gap:4px">
      ...
      <div><b>Total estimé (promo -15%) :</b> ${discounted.toFixed(2)} € TTC</div>
    </div>`;
  }
  // store for Calendly/Stripe
...
function addMADControls(){
  ...
  // Insert MAD duration row after passengers row
